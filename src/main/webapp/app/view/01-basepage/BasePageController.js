...
Ext.define('Hc_Common.view.BasePageController', {
    ...
    saveData: function (options) {
        ...
        if(options.obj_){
        	me.view.subformActivate=false;
        	options.obj_.view.hide();
        }
        var param = {
            url: url,
            method: 'POST',
            success: function (response) {
                ...
                if(options.obj_){
                	me.view.grid.store.reload();
                	//me.view.grid.refreshScroll();	
                	me.view.refreshScroll();
                }
            },
        ...
        if (options.isJson === false) {
        	param.params = Ext.JSON.decode(JSON.stringify(options.data));
        } else {
    ...
    getDataToSave: function (obj,isFormDel) {
        ...
        if(this.windowFlag){
        	if(this.view.subformActivate){
        		me.workObject.batchUrl=this.view.gridAddMasterCustomerUrl;
        	}else{       
        		masterListData.push(me.getGridDirtyData(obj));
        		param.customerName =Ext.util.Format.trim(me.view.subWinTableName).substr(Ext.util.Format.trim(me.view.subWinTableName).lastIndexOf('.') + 1) ;
        		param.masterName = me.view.self.getName().substr(me.view.self.getName().lastIndexOf('.') + 1);
        		param.idField = idField;
        		param.isJson=false;
        		me.workObject.batchUrl=this.view.gridDeleteMasterCustomerUrl;
        		 if (masterListData.length > 0) {
        	            param.masterListData =JSON.stringify(masterListData) ;
        	      }
        		return param;
        	}
        }
