...
Ext.define('Hc_Common.view.BaseSimplePageController', {
    ...
    init: function () {
        ...
        try {
            ...
            if (grid) {
               ...
               // grid.on('celldblclick', me.onEditClick, me);
                grid.on('celldblclick', me.onEditClick_, me);
    ...
    onBtnAddClick: function (btn) {
         ...
         if(windowFlag){
       	 var  new_obj_ =Ext.create( me.newobj, {
       		  me_:this
   			 }); 
        	this.onBtnCancelClick();
       	  var  record=Ext.create(store.model.getName(),{}); 
     		  var  obj_={_flag: 'A'};
     		 for(i=0;i<record.getFields().length-3;i++){
     			 var  name=Ext.util.Format.trim(record.getFields()[i].getName())
           		 var  str;
           		 if((new_obj_.getReferences())[name]){
           			 if(new_obj_.lookupReference(name).is('combo')){
               			 str='{'+name+':\''+new_obj_.lookupReference(name).getDisplayValue(record.get(name))+'\'}';
               			 Ext.apply(obj_, Ext.decode(str))
               			
               		 }else{
               			 str='{'+name+':\''+new_obj_.lookupReference(name).getRawValue(record.get(name))+'\'}';
               			 Ext.apply(obj_,Ext.decode(str))
     		...
     		var newObj_ = grid.store.model.create(obj_);
           	Ext.apply(new_obj_, {
           		record:newObj_,
           		me_:this
           	})
           	me.view.subformActivate=true;
           	new_obj_.show();
         	return;
         }
    ...
    /**提交保存 (批量处理)*/
    onBtnSaveClick: function (btn,obj_) {
        var me = this,
            obj = me.workObject,
            me_;
        
        
        if(Ext.getClassName(obj_)=='Ext.event.Event'){
     	   if(obj_.isEvent){
     		 me_=false;
     	   }
        }else{
        ...
        var param = me.getDataToSave(obj,obj_);

        if (!param)return;

        if (me.beforeSave(param) === false)return;
        me.saveData({srcObj: obj, data: param, btn: btn,obj_:me_,isJson:param.isJson});
    },
